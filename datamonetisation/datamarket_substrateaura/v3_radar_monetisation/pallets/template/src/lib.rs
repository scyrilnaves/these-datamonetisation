...
pub mod pallet {
	...
	pub(super) type FirstAssetBid<T: Config> =
		StorageMap<_, Twox64Concat, (T::Hash, u64), (T::AccountId, BalanceOf<T>), ValueQuery>;
	...
	pub struct GenesisConfig<T: Config> {
		pub assets: Vec<(T::AccountId, T::Hash, BalanceOf<T>, u64, BalanceOf<T>)>,
		pub assetservices: Vec<(T::AccountId, T::Hash, BalanceOf<T>, u64, BalanceOf<T>)>,
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn accept_bid(
			...
			for j in 0..FirstAssetBidCounter {
				let ((Bidder, Bid_Amount)) = Self::get_firstassetbid((asset_id, j));
		...
		pub fn accept_bid_service(
			...
			<FirstAssetServiceBidCounter<T>>::insert(asset_id, new_first_asset_bid_counter);
			for j in 0..FirstAssetBidCounter {
				let ((Bidder, Bid_Amount)) = Self::get_firstassetservicebid((asset_id, j));
				if (Bid_Amount >= bid_threshold_price) {
