  ...
  spec:
    ...
    template:
      ...
      spec:
        ...
        containers:
          - name: substrate-node
            image: cyrilthese/datamonetisation_aura:latest
            resources:
              requests:
                memory: "10Gi"
                cpu: "4"
...
docker pull cyrilthese/datamonetisation_aura:latest >out.log 2>err.log
docker run -it cyrilthese/datamonetisation_aura:latest ./node-template build-spec --disable-default-bootnode --chain local -l error >customSpec.json
#sleep 180
...
#build raw chainSpec
docker run -it -v $(pwd)/customSpec.json:/customSpec.json cyrilthese/datamonetisation_aura:latest ./node-template build-spec --chain=/customSpec.json --raw --disable-default-bootnode -l error >customSpecRaw.json
