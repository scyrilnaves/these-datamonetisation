...
pub mod pallet {
	...
	pub(super) type FirstAssetBid<T: Config> =
		StorageMap<_, Twox64Concat, (T::Hash, u64), (T::AccountId, BalanceOf<T>, Vec<u8>)>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn accept_bid(
			...
			for j in 0..FirstAssetBidCounter {
				let ((Bidder, Bid_Amount, Encrypted_Bid)) =
					Self::get_firstassetbid((asset_id, j)).unwrap();
		...
		pub fn submit_asset_service_review(
			...
			let vehicle_oem = Self::owner_of_assetservice(asset_service_id)
				...
				vehicle_oem == Self::vehicles(submit_asset_service_reviewer.clone()).unwrap(),
		...
		pub fn accept_bid_service(
			...
			for j in 0..FirstAssetBidCounter {
				let ((Bidder, Bid_Amount, Encrypted_Bid)) =
					Self::get_firstassetservicebid((asset_id, j)).unwrap();
				if (Bid_Amount >= bid_threshold_price) {
		...
		pub fn accept_bid_service_privacy(
			...
			<FirstAssetServiceBidCounter<T>>::insert(asset_id, new_first_asset_bid_counter);
			for j in 0..FirstAssetBidCounter {
				let ((Bidder, Bid_Amount, Encrypted_BidAmount)) =
					Self::get_firstassetservicebid((asset_id, j)).unwrap();
				let mut asset = Self::assetservices(asset_id).unwrap();
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn validate_proof_asset(asset_id: T::Hash) -> Result<BTreeSet<T::AccountId>> {
			...
			for i in 1..no_proofs_asset + 1 {
				let (vehicle_id, hash_proof) = Self::get_assetproof((asset_id, i)).unwrap();
				...
				let calculated_hash =
					hashing::sha2_256(vehicledata["jwt"].as_str().unwrap().as_bytes());
