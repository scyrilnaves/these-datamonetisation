...
for ((i = 0; i <= $NBNODES; i++)); do
  ...
  if [[ "$i" -eq 0 ]]; then
    ...
    cmd_add_to_keystore=""
    for ((j = 1; j <= $NBNODES; j++)); do # start 1 => no bootnode
      cmd_add_to_keystore+=$(
        cat <<EOF

                    ./node-template key insert --base-path /datas/substrate-$i --chain local --key-type babe --suri "${Ed25519_arr_secretSeed[j]}";
  ...
  spec:
    ...
    template:
      ...
      spec:
        ...
        containers:
          - name: substrate-node
            image: cyrilthese/datamonetisation_babe:latest
            resources:
              requests:
                memory: "10Gi"
                cpu: "4"
...
docker pull cyrilthese/datamonetisation_babe:latest >out.log 2>err.log
docker run -it cyrilthese/datamonetisation_babe:latest ./node-template build-spec --disable-default-bootnode --chain local -l error >customSpec.json
#echo 'BE READY'
#sleep 500
#chainintialSpec=$(cat customSpec.json | sed '$d') #get file content and remove last line (has some unwanted output)
#echo $chainintialSpec >customSpec.json            #write file content
#echo ' }' >>customSpec.json                       # addd end of config
#sleep 30
###################### make palletbabe authorities (Sr25519 keys)
palletbabe_authorities="["
for ((i = 1; i <= $NBNODES; i++)); do # start 1 => no bootnode
  palletbabe_authorities+=$(
    cat <<EOF
      [
    "${Sr25519_arr_ss58PublicKey[i]}",100000
      ],
...
palletbabe_authorities=${palletbabe_authorities::-1} #DON'T FORGET TO REMOVE LAST CHARACTER: ${palletbabe_authorities::-1}
palletbabe_authorities+="]"
palletbabe_authorities=$(echo "$palletbabe_authorities" | jq -c) #format json to a one line
###################### end make palletbabe authorities
...
#jq
chainSpec=$(cat customSpec.json)
chainSpec=$(echo $chainSpec | jq ".genesis.runtime.babe.authorities = ${palletbabe_authorities}")
chainSpec=$(echo $chainSpec | jq ".genesis.runtime.grandpa.authorities = ${palletGrandpa_authorities}")
chainSpec=$(echo $chainSpec | jq '.name = "Data Monetisation Chain"')
chainSpec=$(echo $chainSpec | jq '.id = "local_testnet"')

echo $chainSpec | jq >customSpec.json #write changes to file

#build raw chainSpec
docker run -it -v $(pwd)/customSpec.json:/customSpec.json cyrilthese/datamonetisation_babe:latest ./node-template build-spec --chain=/customSpec.json --raw --disable-default-bootnode -l error >customSpecRaw.json
#chainInitialSpecRaw=$(cat customSpecRaw.json | sed '$d') #get file content and remove last line (has some unwanted output)
#echo $chainInitialSpecRaw >customSpecRaw.json            #write file content
#echo ' }' >>customSpecRaw.json
    ...
    template:
      ...
      spec:
        ...
        - ip: "185.52.32.4"
          ...
          - "radarapi.unice.cust.tasfrance.com"
        containers:
        - name: datamarket-client
          image: cyrilthese/datamarket-client:latest
          command:
            - "sleep"
            - "604800"
          resources:
  ...
  spec:
    ...
    template:
      metadata:
        labels:
          name: vehicleapi
          serviceSelector: vehicleapi-node
      spec:
        hostAliases:
        - ip: "185.52.32.4"
          hostnames:
        ...
        securityContext:
          fsGroup: 101
        containers:
          - name: vehicleapi-node
            image: cyrilthese/oem-api:latest
            env:
              - name: JAVA_ARGS
            ...
            ports:
              - name: http
                containerPort: 8080
            volumeMounts:
              - name: vehicleapi-data
                mountPath: /datas/vehicleapi
             
        volumes:
          - name: vehicleapi-data
            persistentVolumeClaim:
              claimName: vehicleapi-data
EOF

# define service for node
cat <<EOF

#---------------------------------=NODES SERVICES vehicleapi=---------------------------------------
- apiVersion: v1
  kind: Service
  metadata:
    name: vehicleapi
...
# define volume for node
cat <<EOF
#---------------------------------=NODES PERSISTANT VOLUME vehicleapi=---------------------------------------
- apiVersion: v1
  kind: PersistentVolume
  metadata:
    name: vehicleapi
  ...
  spec:
    ...
    persistentVolumeReclaimPolicy: Recycle
    hostPath:
      path: "/datas/vehicleapi"
EOF

# define volume claim for node
cat <<EOF
#--------------------------=PERSISTENT VOLUME CLAIM vehicleapi=------------------------------
  ...
  spec:
    ...
    template:
      ...
      spec:
        ...
        containers:
          - name: radarapi-node
            image: cyrilthese/radar-api:latest
            env:
              - name: JAVA_ARGS
                value: "-Xmx6000M -Xms5096M"
            resources:
            ...
            volumeMounts:
              - name: radarapi-data
                mountPath: /datas/radarapi
             
        volumes:
          - name: radarapi-data
            persistentVolumeClaim:
              claimName: radarapi-data
EOF

# define service for node
cat <<EOF

#---------------------------------=NODES SERVICES radarapi=---------------------------------------
- apiVersion: v1
  kind: Service
  metadata:
    name: radarapi
...
# define volume for node
cat <<EOF
#---------------------------------=NODES PERSISTANT VOLUME radarapi=---------------------------------------
- apiVersion: v1
  kind: PersistentVolume
  metadata:
    name: radarapi
...
# define volume claim for node
cat <<EOF
#--------------------------=PERSISTENT VOLUME CLAIM radarapi=------------------------------
...
#--------------------------=HTTP SERVICE FOR VEHICLE API, RADAR API NODE=--------------------------------

- apiVersion: v1
  kind: Service
  metadata:
    name: vehicleapi-http-service
    namespace: substrate-net
  spec:
    type: ClusterIP
    selector:
